...
class Entity:
    ...
    def before_update(self, dt: float) -> None:
        if self._current_animation is not None:
            ...
            # print("lakjdflöakjsdflöakfjdölakjsdfölakjsdlfökasldfkjsalökjdflaösfjkdlöaskjdflöasjkdflöasjdflök")

            # print(self.last_animation_time, self.animations_time[self.current_animation])
            if self._last_animation_time >= self._animations_time[self._current_animation]:
                self._current_sprite_index += 1
                self._current_sprite_index %= len(self._animations[self._current_animation])
                self.surface = self._animations[self._current_animation][self._current_sprite_index]
    ...
    def collide(self, other_entity: Entity) -> bool:
        ...
        other_pos = pygame.Vector2(other_pos.x + other_entity.box_offset[0], other_pos.y + other_entity.box_offset[1])
        own_pos = pygame.Vector2(own_pos.x + self.box_offset[0], own_pos.y + self.box_offset[1])
    ...
    def collide_point(self, point: pygame.Vector2) -> bool:
        ...
        rx, ry, rw, rh = own_pos.x + self.box_offset[0], own_pos.y + self.box_offset[1], self.box[0], self.box[1]
    ...
    def resolve_x(self, velocity_x: int, other_entity: Entity) -> None:
        if velocity_x > 0:
            target_x = other_entity.world_position()[0] - self.box[0] - self.box_offset[0]

        elif velocity_x < 0:
            target_x = other_entity.world_position()[0] + other_entity.box[0] - self.box_offset[0]
    ...
    def resolve_y(self, velocity_y: int, other_entity: Entity) -> None:
        if velocity_y > 0:
            target_y = other_entity.world_position()[1] - self.box[1] - self.box_offset[1]

        elif velocity_y < 0:
            target_y = other_entity.world_position()[1] + other_entity.box[1] - self.box_offset[1]
    ...
    def switch_to_animation(self, name: str) -> None:
        ...
        self.__last_animation_time = 0.0